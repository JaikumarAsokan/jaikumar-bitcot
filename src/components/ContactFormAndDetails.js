    ...
    <div className="modal">
      <div className="modal-content">
        <span className="close" onClick={onClose}>&times;</span>
          <>
            <h2>{mode === 'add' ? 'Add Contact' : mode === 'edit' ? 'Edit Contact' : 'View Contact'}</h2>
            <form onSubmit={handleSubmit}>
             <fieldset disabled={mode === 'view'} className='fieldset'>
              ...
              <input type='text' pattern="[0-9]*" inputmode="numeric" name="mobile" value={formContact.mobile} onChange={handleChange} />
              </label>
              <label><strong>Email</strong> 
                <input type="email" name="email" value={formContact.email} onChange={handleChange} />
              </label>
              { mode !== 'view'&&  <button type="submit">{mode === 'add' ? 'Add Contact' : 'Update Contact'}</button>}
              </fieldset>
            </form>
          </>
